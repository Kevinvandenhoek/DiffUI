...
public protocol DFVisibilityAnimatable {
    
    func set(visible: Bool, animated: Bool)
}

public extension DFVisibilityAnimatable {
...
public extension DFVisibilityAnimatable where Self: UIView {
...
extension Array: DFVisibilityAnimatable where Element: UIView {
    ...
    private var notVisibilityAnimatableSubviews: [UIView] {
        return filter {
            if let viewContainer = $0 as? DFViewContainer {
                ...
                return viewContainer.containedView is DFVisibilityAnimatable == false
            } else {
                return $0 is DFVisibilityAnimatable == false
            }
        }
    }
    
    private var visibilityAnimatableSubviews: [DFVisibilityAnimatable] {
        return compactMap {
            if let viewContainer = $0 as? DFViewContainer {
                return viewContainer.containedView as? DFVisibilityAnimatable
            } else {
                return $0 as? DFVisibilityAnimatable
            }
        }
    }
